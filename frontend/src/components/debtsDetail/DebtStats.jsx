...
export default function DebtStats({ debts = [], members = [], currentUser }) {
  ...
  const CenterMemberAvatar = ({ member, role, amount }) => {
    if (!member)
      return <User className="w-12 h-12 text-zinc-500 sm:w-16 sm:h-16" />;
    const isCreditor = role === "債主";
    const colorClass = isCreditor ? "border-green-500" : "border-red-500";
    return (
      <div className="flex flex-col items-center">
        <Avatar
          className={`w-12 h-12 sm:w-16 sm:h-16 md:w-20 md:h-20 border-4 ${colorClass} shadow-lg ring-4 ring-zinc-900`}
        >
          <AvatarImage src={member.photoUrl} alt={member.displayName} />
          <AvatarFallback className="bg-zinc-600 text-white text-lg sm:text-xl">
            {member.displayName?.[0]}
          </AvatarFallback>
        </Avatar>
        <p className="mt-2 text-white font-bold text-sm sm:text-base text-center">
          {member.displayName}
        </p>
        <Badge
  ...
  return (
    <div className="flex flex-col gap-6">
      {/* 💰群組未結總覽 */}
      <Card className="bg-gradient-to-br from-zinc-700 to-zinc-900 border-zinc-600 p-4 sm:p-6 text-white rounded-2xl shadow-2xl col-span-2 overflow-auto">
        <h2 className="text-xl sm:text-2xl font-extrabold mb-4 text-center border-b border-zinc-600 pb-2 text-yellow-400">
          💰 群組未結總覽
        </h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-center">
          <div className="p-3 bg-zinc-800 rounded-xl border border-zinc-700">
            ...
            <p className="text-2xl sm:text-3xl font-mono mt-1 text-red-300">
          ...
          <div className="p-3 bg-zinc-800 rounded-xl border border-zinc-700">
            ...
            <p className="text-2xl sm:text-3xl font-mono mt-1 text-green-300">
              NT${stats.totalReceive.toFixed(0)}
            </p>
          </div>
          <div className="p-3 bg-zinc-800 rounded-xl border border-zinc-700">
            <p className="text-sm text-red-500 font-medium">我的應付</p>
            <p className="text-xl sm:text-2xl font-mono mt-1 text-red-400">
              NT${myDebt.toFixed(0)}
            </p>
          </div>
          <div className="p-3 bg-zinc-800 rounded-xl border border-zinc-700">
            <p className="text-sm text-green-500 font-medium">我的應收</p>
            <p className="text-xl sm:text-2xl font-mono mt-1 text-green-400">
      ...
      {/* 🧍最大債主與負債者區塊（並排） */}
      <Card className="bg-zinc-800 border border-zinc-700 rounded-2xl p-4 sm:p-6 shadow-xl">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 items-start">
          ...
          <div className="flex flex-col items-center text-center">
            ...
            <p className="text-sm sm:text-base text-zinc-400 mb-3">
              這位成員收回最多
            </p>
            <CenterMemberAvatar
